...
static int js_transport_write_message_newline(JSDebuggerInfo *info,
...
{
  JSValue stringified =
      JS_JSONStringify(info->ctx, value, JS_UNDEFINED, JS_UNDEFINED);
  size_t len;
...
static JSValue js_transport_new_envelope(JSDebuggerInfo *info,
                                         const char *type)
{
...
static JSValue js_get_scopes(JSContext *ctx, int frame)
{
  ...
  JS_SetPropertyStr(ctx, local, "reference",
                    JS_NewInt32(ctx, (frame << 2) + 1));
  ...
  JS_SetPropertyStr(ctx, closure, "reference",
                    JS_NewInt32(ctx, (frame << 2) + 2));
  ...
  JS_SetPropertyStr(ctx, global, "reference",
                    JS_NewInt32(ctx, (frame << 2) + 0));
                                          ...
                                          JSValue var, JSValue var_val)
{
  ...
  else if (JS_IsObject(var_val))
  {
    ...
    if (JS_IsUndefined(found))
    {
      ...
      JS_SetPropertyUint32(ctx, state->variable_pointers, pl,
                           JS_NewInt32(ctx, reference));
    }
    else
    {
  ...
  JS_SetPropertyStr(ctx, var, "variablesReference",
                    JS_NewInt32(ctx, reference));
}

static void js_debugger_get_value(JSContext *ctx, JSValue var_val, JSValue var,
...
static void js_free_prop_enum(JSContext *ctx, JSPropertyEnum *tab,
                              uint32_t len)
{
  uint32_t i;
  if (tab)
...
static uint32_t js_get_property_as_uint32(JSContext *ctx, JSValue obj,
                                          const char *property)
{
...
static void js_process_request(JSDebuggerInfo *info,
                               struct DebuggerSuspendedState *state,
                               JSValue request)
{
  JSContext *ctx = info->ctx;
...
static void js_process_breakpoints(JSDebuggerInfo *info, JSValue message)
{
  ...
  JS_SetPropertyStr(ctx, path_data, "dirty",
                                        ...
                                        const uint8_t *cur_pc)
{
  ...
  do
  {
    ...
    if (message_length > info->message_buffer_length)
    {
      ...
      info->message_buffer =
          js_malloc_rt(JS_GetRuntime(ctx), message_length + 1);
    ...
    {
      js_process_request(info, &state,
                         JS_GetPropertyStr(ctx, message, "request"));
      // done_processing = 1;
    }
    ...
    {
      js_process_breakpoints(info,
                             JS_GetPropertyStr(ctx, message, "breakpoints"));
    }
...
static void js_debugger_context_event(JSContext *caller_ctx,
                                      const char *reason)
{
  ...
  JS_SetPropertyStr(ctx, event, "reason", JS_NewString(ctx, reason));
  JS_SetPropertyStr(ctx, event, "thread",
                    JS_NewInt64(ctx, (int64_t)caller_ctx));
  js_transport_send_event(info, event);
}
...
void js_debugger_check(JSContext *ctx, const uint8_t *cur_pc)
{
  ...
  int at_breakpoint = js_debugger_check_breakpoint(
...
{
  return js_debugger_info(rt)->inDebug == 1;
}
